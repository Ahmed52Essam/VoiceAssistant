...
from commands import ReadClipboardCommand

class TestReadClipboardCommand(unittest.TestCase):
    def setUp(self):
        self.tts = MagicMock(spec=TextToSpeech)
        self.recognizer = MagicMock(spec=SpeechRecognizer)
        self.command = ReadClipboardCommand()

    def test_match_expected_command(self):
        self.assertTrue(self.command.match("read clipboard"))

    def test_match_invalid_command(self):
        self.assertFalse(self.command.match("copy text"))

    def test_execute(self):
        result = self.command.execute("read clipboard", self.tts, self.recognizer)
