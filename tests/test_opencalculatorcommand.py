...
from commands import OpenCalculatorCommand

class TestOpenCalculatorCommand(unittest.TestCase):
    def setUp(self):
        self.tts = MagicMock(spec=TextToSpeech)
        self.recognizer = MagicMock(spec=SpeechRecognizer)
        self.command = OpenCalculatorCommand()

    def test_match_expected_command(self):
        self.assertTrue(self.command.match("open calculator"))

    def test_match_invalid_command(self):
        self.assertFalse(self.command.match("open notepad"))

    def test_execute(self):
        result = self.command.execute("open calculator", self.tts, self.recognizer)
